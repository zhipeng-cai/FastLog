...
input_file = '../output/predictions/positions.csv'
output_dir = '../output/predictions'
output_file = output_dir + '/statements_beam_search.csv'
...
if __name__ == '__main__':
    ...
    input_texts = input_texts['Pred_Position']
    ...
    with torch.no_grad():
        ...
        for index, input_text in input_texts.items():
            ...
            if len(source_ids) <= input_max_length:
    ...
    predictions = [post_process_punctuation(pred) for pred in predictions]
    levels = [get_log_level(pred) for pred in predictions]
    messages = [get_log_message(pred) for pred in predictions]

    data = pd.DataFrame()
    data["LogStatement"] = predictions
    data["Level"] = levels
    cat_dict = {'trace': 0, 'debug': 1, 'info': 2, 'warn': 3, 'error': 4, 'fatal': 5}
    data["Level"] = data["Level"].map(cat_dict).fillna(-1).astype(int)
